...
* 登入流程 **必須維持 URL query login_ticket（登入交換憑證）**

---

## 絕對不可變更（Hard Constraints）

1. Vue2 登入仍以 URL query login_ticket（登入交換憑證）為唯一入口
2. Vue2 仍為「是否已登入」的最終判定者
3. 不可移除 URL query login_ticket
...
1. Vue3 可產生 login_ticket 並帶入 iframe URL
...
1. Vue3 取得 login_ticket（登入交換憑證）
2. Vue3 iframe URL 帶入 login_ticket
3. Vue2 解析 URL login_ticket 並依原流程登入
